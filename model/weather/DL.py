...
class WeatherLoader:
    ...
    def cleanData(self):
        ...
        for location_name, location_data in self.data.items():
            ...
            # daily_averages = location_data.groupby('DATE').mean()

            # daily_averages.reset_index(inplace=True)
            
            # self.data[location_name] = daily_averages
    ...
    def getTestingData(self):
        ...
        for location_name, location_data in self.data.items():
            # test_data[location_name] = location_data[(location_data['DATE'].dt.year == 2020) & 
            #                              (location_data['DATE'].dt.month >= 6) & 
            #                              (location_data['DATE'].dt.month <= 10)]

            # self.data[location_name] = self.data[location_name][~((self.data[location_name]['DATE'].dt.year == 2020) & 
            #                                          (self.data[location_name]['DATE'].dt.month >= 6) & 
            #                                          (self.data[location_name]['DATE'].dt.month <= 10))]
